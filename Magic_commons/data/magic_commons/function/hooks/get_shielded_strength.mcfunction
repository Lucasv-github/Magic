...
scoreboard players operation Temp reg_1 = @s[tag=can_use,tag=using] cumulative_halve_amount_hold
scoreboard players operation Temp reg_1 = @s[tag=o_can_use,tag=o_using] o_cumulative_halve_amount_hold
execute if score Temp reg_2 matches 1.. run scoreboard players operation Temp reg_1 -= Temp reg_2
