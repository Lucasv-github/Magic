...
#Snap back
#To self
execute if score Temp reg_1 < @e[tag=getting_cut,limit=1] current_held run scoreboard players operation Temp reg_2 = @s weave_owner_player_id
execute if score Temp reg_1 < @e[tag=getting_cut,limit=1] current_held as @a[tag=using,tag=can_use] if score @s player_id = Temp reg_2 at @s run damage @s 1
execute if score Temp reg_1 < @e[tag=getting_cut,limit=1] current_held as @a[tag=using,tag=can_use] if score @s player_id = Temp reg_2 run clear @s minecraft:carrot_on_a_stick[custom_data~{Magic:6}] 4

#To opponent
execute if score Temp reg_1 > @e[tag=getting_cut,limit=1] current_held run scoreboard players operation Temp reg_2 = @e[tag=getting_cut,limit=1,sort=arbitrary,tag=actively_held] weave_owner_player_id
execute if score Temp reg_1 > @e[tag=getting_cut,limit=1] current_held as @a[tag=using,tag=can_use] if score @s player_id = Temp reg_2 at @s run damage @s 1
execute if score Temp reg_1 > @e[tag=getting_cut,limit=1] current_held as @a[tag=using,tag=can_use] if score @s player_id = Temp reg_2 run clear @s minecraft:carrot_on_a_stick[custom_data~{Magic:6}] 4
