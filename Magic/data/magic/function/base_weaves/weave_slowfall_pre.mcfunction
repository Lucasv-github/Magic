...
execute store result storage magic:slowfall amount int 1 run scoreboard players get @s reg_1
execute as @e if score @s entity_id = Temp reg_1 run function magic:base_weaves/weave_slowfall with storage magic:slowfall

function magic:weave_processing/advance_read_index

data remove storage magic:slowfall amount

tag @s add weave_slowfall