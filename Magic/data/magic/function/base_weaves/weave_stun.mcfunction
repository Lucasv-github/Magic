...
$execute at @s as @a[tag=using] if score @s player_id = Temp reg_1 run clear @s[nbt=!{SelectedItem:{id:"minecraft:ender_eye",components:{"minecraft:custom_data":{Magic:6}}}}] ender_eye $(size)