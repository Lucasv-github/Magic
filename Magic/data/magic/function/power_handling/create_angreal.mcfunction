...
execute as @s[tag=create_angreal_top_temp] if items entity @s contents #magic:prevent_placement run data modify entity @s Item.components merge value {attribute_modifiers:[{type:block_interaction_range,amount:-1,operation:add_multiplied_base,id:"1744537967229"}],tooltip_display:{hidden_components:[attribute_modifiers]}}
