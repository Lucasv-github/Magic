...
#The sending TODO replace with giving book
execute in minecraft:overworld run setblock 0 0 0 oak_sign{front_text:{messages:['{"nbt":"text[]","storage":"magic:give_current_weave_build","interpret":true,"separator":"","color":"white"}','{"text":""}','{"text":""}','{"text":""}']}} destroy
...
#data modify entity @e[limit=1,sort=nearest, tag=give_current_weave] Items[0].components.minecraft:lore[0] set from block 0 0 0 front_text.messages[0]
#data modify entity @e[limit=1,sort=nearest, tag=give_current_weave] Items[0].components.minecraft:lore append from block 0 0 0 front_text.messages[0]

#Need to be above to prevent triggering directly after
...
#Clean for next
execute if score Temp reg_1 matches 0 run data merge storage magic:give_current_weave_build {text:[]}
execute if score Temp reg_1 matches -1 run data merge storage magic:give_current_weave_build {text:[]}

execute unless score Temp reg_1 matches 0 run function magic:give_current_weave_iteration with storage magic:give_current_weave_iteration