#TODO cut soft: tied off or snap back if less than opponent, else nothing
...
#Need to hold more than opponent
execute if score Temp reg_1 < @e[tag=getting_cut,limit=1,sort=arbitrary] current_held run tag @e remove getting_cut

#Snapback if worked no matter how much
execute if entity @e[tag=getting_cut,limit=1,sort=arbitrary,tag=actively_held] run scoreboard players operation Temp reg_1 = @s player_id
execute if entity @e[tag=getting_cut,limit=1,sort=arbitrary,tag=actively_held] as @a[tag=using,tag=can_use] if score @s player_id = Temp reg_1 run damage @s 1
execute if entity @e[tag=getting_cut,limit=1,sort=arbitrary,tag=actively_held] as @a[tag=using,tag=can_use] if score @s player_id = Temp reg_1 run clear @s minecraft:ender_eye 4
