...
execute at @s[tag=build_bridge] run scoreboard players operation @e[sort=nearest, type=minecraft:armor_stand, tag=destroy, distance=..5] weave_execute_random_number = @e[type=minecraft:snowball,tag=ray, tag=build_bridge] weave_execute_random_number
