...
$execute unless entity @e[tag=tar_chunk_loaded_marker,name=X_$(x)Y_$(y)] in magic:tar run summon minecraft:armor_stand $(x) 100 $(y) {NoGravity:1b,Tags:["tar_chunk_loaded_not_copied"]}
$execute unless entity @e[tag=tar_chunk_loaded_marker,name=X_$(x)Y_$(y)] in magic:tar run forceload add $(x) $(y)
