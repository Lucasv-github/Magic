...
#Sword AFAFFF
execute at @s[tag=current_target,scores={t_1=1,t_2=3,t_3=1,t_4=3,t_5=3,t_6=3,t_7=0}] as @a[tag=can_use] if score @s player_id = @e[limit=1, tag=current_target] player_id unless entity @s[nbt={Inventory:[{id:"minecraft:golden_sword",tag:{Force:20}}]}] run give @s golden_sword{Unbreakable:1,display:{Name:'[{"text":"Fire sword","italic":false}]',Lore:['[{"text":"Force","italic":false}]']},Enchantments:[{id:fire_aspect,lvl:20},{id:sharpness,lvl:20}],HideFlags:5,Force:20} 1
