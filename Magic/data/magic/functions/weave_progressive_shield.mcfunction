...
execute as @s unless score @s state matches 2 run scoreboard players remove @a[tag=progressive_shield_me,scores={progressive_shielded=1.., shilded=0}] progressive_shield_current 10
execute as @s unless score @s state matches 2 run scoreboard players operation @a[tag=progressive_shield_me,scores={progressive_shielded=1.., shilded=0}] cumulative_halve_amount_hold = @a[tag=progressive_shield_me,scores={progressive_shielded=1.., shilded=0}] progressive_shield_current
...
execute if entity @a[tag=can_use, tag=progressive_shield_me, limit=1,scores={shilded=1..}] as @a[tag=can_use,tag=using] if score @s player_id = Temp reg_2 at @s run playsound minecraft:block.anvil.place ambient @s
