...
execute as @a[tag=magic_help_interactive_tutorial_wait_bind] at @s as @e[type=villager,tag=magic_help_interactive_tutorial_villager,sort=nearest,limit=1] if predicate {"type":"minecraft:entity_properties","entity":"this","predicate":{"minecraft:vehicle":{"minecraft:entity_type":"minecraft:armor_stand"}}} as @a[limit=1,sort=nearest] run function magic_help:interactive_tutorial_stage_6
